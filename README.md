...
- `internal/pipeline` - a single goroutine ticks collectors at `interval`, applies decorators (peak tracker), and fans the snapshot out to subscriber channels with non-blocking sends. Slow consumers can drop samples but cannot stall the producer.
